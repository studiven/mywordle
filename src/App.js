...
function App() {
  ...
  // Ref damit async Funktionen immer die aktuelle Sprache sehen
  ...
  useEffect(() => {
    ...
    if (gameMode === "daily") {
      getDailyWord(languageRef.current).then(word => {
    ...
    if (gameMode === "unlimited") {
      loadValidTargetWord(languageRef.current);
    }
  ...
  async function loadValidTargetWord(lang) {
    ...
    while (!found && currentTries < retries) {
      try {
        ...
        if (!randomRes.ok) throw new Error(`API Fehler: ${randomRes.status}`);
        ...
        if (!dictRes.ok) throw new Error(`API Fehler: ${dictRes.status}`);
  ...
  async function getDailyWord(lang) {
